...
type PixelBlastProps = {
  ...
  edgeFade?: number;
  noiseAmount?: number;
  /** 每次递增时在画面中心触发一次像素波纹（用于校准完成等） */
  burstCount?: number;
  /**
   * 若提供，波纹中心对齐该元素在视口中的几何中心，再映射到 WebGL 缓冲区坐标。
   * 校准页可指向整页根节点，使圆心与页面（含顶栏）中心一致。
   */
  burstCenterTargetRef?: React.RefObject<HTMLElement | null>;
};
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    const tryBurst = () => {
      ...
      const t = threeRef.current;
      if (!t) {
        if (tries++ < 120) requestAnimationFrame(tryBurst);
        return;
      }
      const canvasEl = t.renderer.domElement;
      ...
      // 与 setSize 一致，避免 uResolution 与 drawing buffer 短暂不同步导致波纹偏移
      t.uniforms.uResolution.value.set(bufW, bufH);

      const rect = canvasEl.getBoundingClientRect();
      const target = burstCenterTargetRef?.current;
      ...
      if (
        ...
        const clientX = br.left + br.width / 2;
        const clientY = br.top + br.height / 2;
        fx = (clientX - rect.left) * scaleX;
        fy = (rect.height - (clientY - rect.top)) * scaleY;
      } else {
        // 直接用缓冲区中心（gl_FragCoord 空间，原点在左下），避免 CSS/rect 换算误差
        fx = bufW * 0.5;
        fy = bufH * 0.5;
      }
  ...
  }, [burstCount, burstCenterTargetRef]);
